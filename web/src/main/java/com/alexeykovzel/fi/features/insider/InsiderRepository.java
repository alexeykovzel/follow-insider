...
public interface InsiderRepository extends JpaRepository<Insider, String> {
    ...
    // TODO: Solve case if positions is null.
    ...
    @Query(value = "SELECT DISTINCT ON (i.cik) i.name, string_agg(ip.positions, ', ') AS positions, " +
            ...
            "WHERE i.cik = ip.insider_cik " + // <--------- here is a bug !!!
            "  AND s.symbol = :symbol " +
            "  AND s.cik = f.stock_cik " +
            "  AND s.cik = i.stock_cik " +
            ...
            nativeQuery = true)
    Collection<Insider.TableView> findTableViewsByStock(String symbol);
}
