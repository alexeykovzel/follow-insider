...
public class StockNewsFactory {
    ...
    public StockStory exploreRecentBuys(Stock stock) {
        ...
        List<Trade> trades = tradeRepository.findRecentBuysBuyCik(stock.getCik(), from);
        if (trades.isEmpty()) return null;
        Trade trade = getTradeOfHighestValue(trades);
        ...
        String tradeValue = StringUtils.formatNumber(trade.getTotalValue());
    ...
    private Trade getTradeOfHighestValue(List<Trade> trades) {
        ...
        for (int i = 1; i < trades.size(); i++) {
            if (trades.get(i).getTotalValue() > trade.getTotalValue()) {
                trade = trades.get(i);
            }
        }
        return trade;
